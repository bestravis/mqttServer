...
import com.youxuepai.mqtt.server.common.message.DupPubRelMessageStore;
import com.youxuepai.mqtt.server.common.message.DupPublishMessageStore;
...
public class Connect {
	...
	public Connect(ISessionStoreService sessionStoreService, ISubscribeStoreService subscribeStoreService,
	...
	public void processConnect(Channel channel, MqttConnectMessage msg) {
		...
		String cilentId = msg.payload().clientIdentifier();
		if(cilentId != null && Pattern.matches("stu_.*",cilentId)){
			List<SubscribeStore> subscribeStores = subscribeStoreService.search("/sys/online");
			subscribeStores.forEach(subscribeStore->{
				if (sessionStoreService.containsKey(subscribeStore.getClientId())) {
					// 订阅者收到MQTT消息的QoS级别, 最终取决于发布消息的QoS和主题订阅的QoS
					JSONObject jsonObject = new JSONObject();
					jsonObject.put("userId",cilentId);
					...
					MqttPublishMessage publishMessage = (MqttPublishMessage) MqttMessageFactory.newMessage(
							...
							new MqttPublishVariableHeader("/sys/online", 0), Unpooled.buffer().writeBytes(jsonObject.toJSONString().getBytes()));
					LOGGER.debug("PUBLISH - clientId: {}, topic: {}, Qos: {}", subscribeStore.getClientId(), "/sys/online", 0);
